	...
	<form action="" method="post" class="form form-horizontal" enctype="multipart/form-data">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-4 col-sm-3">
				<input type="text" class="input-text" value="{{ $in->name }}" placeholder="" id="username" name="username">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<select class="select" size="1" name="category">
					...
					<option value="{{ $ca->id }}" @if($in->cname == $ca->name) selected @endif >{{ $ca->name }}</option>
					@endforeach
			</select>
			</span>
			</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-4 col-sm-3">
				<input type="text" class="input-text" value="{{ $in->money }}" placeholder=""  name="money">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>店铺地址</label>
			<div class="formControls col-xs-4 col-sm-3">
				<input type="text" class="input-text" value="{{ $in->address }}" placeholder=""  name="address">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>收款账号</label>
			<div class="formControls col-xs-4 col-sm-3">
				<input type="text" class="input-text" value="{{ $in->account }}" name="account">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>店铺照片</label>
			<div class="formControls col-xs-8 col-sm-9"> <span class="btn-upload form-group">
				<input class="input-text upload-url" type="text" name="uploadfile" value="{{ $in->photo }}" style="width:200px">
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>配送范围</label>
				...
				@foreach($district as $dis)
					@foreach($dis as $di)
					<div style="padding-bottom:10px;" name="addre{{ $di->id }}" class="formControls col-xs-8 col-sm-9"> <span class="btn-upload form-group">
							<input class="input-text upload-url" type="text" name="uploadfile" value="{{ $di->name }}" style="width:200px">
							<a href="javascript:void();" onclick="del({{ $di->id }},{{ $in->id }})" class="btn btn-primary radius upload-btn"><i class="Hui-iconfont">&#xe6a6;</i> 删除地址</a>
							<!-- <input type="file" name="file" multiple  class="input-file"> -->
						</span> 
					</div>
