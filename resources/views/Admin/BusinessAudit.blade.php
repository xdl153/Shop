		...
		<table class="table table-border table-bordered table-bg table-sort">
			...
			<tbody>
				...
				<tr class="text-c">
					...
					<td>{{ $p->id }}</td>
					<td width="130">{{ $p->name }}</td>
					<td>{{ $p->username }}</td>
					<td>{{ $p->phone }}</td>
					<td>{{ $p->email }}</td>
                                        @if ( $p->examine == 1)
                                            <td>状态　:　审核中　操作　:<a onclick="operation({{ $p->id }},2);">通过</a></td>
                                        @elseif ( $p->examine == 2)
                                            <td>状态　:　通过　操作　:<a onclick="operation({{ $p->id }},3);">不通过</a></td
                                        @elseif ( $p->examine == 3)
...
<script type="text/javascript" src="{{ asset('Admin/lib/jquery-1.8.3.min.js') }}"></script> 
<script>
